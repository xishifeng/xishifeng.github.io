'use strict';
var _shuilouGuan = 'http://cq.sanguosha.com/bbs/forum.php?mod=viewthread&tid=210026&highlight=%E6%B0%B4%E6%A5%BC' //官方论坛==>2018年3月
var _shuilou360 = 'http://bbs.u.360.cn/thread-7693393-1-1.html' //360论坛==>2018年4月
...
function forumGuanQianglou(obj) {
	...
	if(_arr.length ^ 0){
		...
		if(_arr.length < 10) {
			...
			if(_domTemp.length >= obj['floorWei'] && _domTemp[obj['floorWei'] - 1].innerHTML === 'man17') {
				console.log('success ==> ' + _lastNum + '楼左右');
				return false;
			} else {
				if((_lastNum + 1) % 10 === obj['floorWei']) {
					document.querySelector('#fastpostmessage').innerText = obj['content'];
					document.querySelector('#fastpostsubmit').click();
					window.location.reload();
				} else {
					setTimeout(function() {
...
function forum360Sign() {
	//360论坛签到
	var _clickBtn = document.getElementById('daily_click');
	if(_clickBtn.innerHTML == '已签到'){
